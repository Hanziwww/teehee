...
fn main() {
    ...
    fragment_includes.push_str(&format!("const FRAGMENTS: [&[u8]; {}] = [", num_real_fragments));
    for i in 0..num_real_fragments {
        if i > 0 {
            fragment_includes.push_str(", ");
        }
    ...
    fs::write(
        fragments_dir.join("metadata.rs"),
        fragment_includes,
    )
    .unwrap();
    
    // Step 10: Export metadata as env vars for runtime configuration
    println!("cargo:rustc-env=TEEHEE_NUM_FRAGMENTS={}", num_real_fragments);
    ...
    println!("cargo:warning=Build security: {} real fragments + {} decoys generated", 
